...
func (s *SearchMessagesState) Help() string {
	...
	return "Type to search  ↑/↓: navigate  Enter: go to message  Esc: close"
}
...
func (s *SearchMessagesState) Render() string {
	...
	if s.Query == "" {
		...
		// Scroll indicators
		if s.ScrollOffset > 0 {
			resultsSection += "\n" + lipgloss.NewStyle().Foreground(ColorTextMuted).Render("  ↑ more above")
		}
		...
		if visibleEnd < len(s.Results) {
			resultsSection += "\n" + lipgloss.NewStyle().Foreground(ColorTextMuted).Render("  ↓ more below")
		}
	}
